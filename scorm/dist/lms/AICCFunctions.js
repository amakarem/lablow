...
//these variables come from and can go straight to the LMS
...
var AICC_Entry = AICC_ENTRY_FLAG_DEFAULT;				//must be the default b/c absence of entry flag indicates "review"
...
//these arrays contain a top level array index by the obj index, these positions contain subarrays with these indices
...
function AICC_CommitData(){
	...
	if (IsThereDirtyAICCData()){
		...
		if ( /*AICC_CourseID != "" && */ AICC_aryInteractions.length > 0){
			...
			//MR - 5/31/05 - Saba won't always accept two request if they come immediately after one another. Introduce an artificial delay between
...
function KillTime(){
	...
	the debug window) to separate the requests.
	*/
...
function GetIndexFromAICCName(strLineName){
	...
	//find period 
		//if found period
		//get all characters after period
...
function AICC_ExtractSingleScoreFromObjective(strLineValue){
	...
	//AICC objectives can have multiple score sets (raw,max,min) separated by semi-colons,they represent the various attempts at an objective
