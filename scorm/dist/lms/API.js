	...
	if (blnStandAlone){
		...
		if (strLMSStandard.toUpperCase() == "AUTO"){
	        ...
	        if(strTemp!=null && strTemp!="")
	        {
				...
				/*
				We wrap the API grabbers in try blocks because we don't know what kind of environment we were
...
function ExecFinish(ExitType){
	...
	if (blnLoaded && ! blnCalledFinish ){
		...
		if (blnReachedEnd){
			WriteToDebug("Reached End, overriding exit type to FINISH");
...
function ShowDebugWindow(){
	...
	winDebug = window.open("blank.html", "Debug", "width=600,height=300,resizeable,scrollbars");
...
//public
//combines API error information with the Standard's error information
...
function RecordNumericInteraction(strID, strResponse, blnCorrect, strCorrectResponse, strDescription, intWeighting, intLatency, strLearningObjectiveID){
	...
	if (! IsValidDecimal(strResponse)){
		WriteToDebug("ERROR - Invalid Response, not a valid decimal");
	...
	if (strCorrectResponse != undefined && strCorrectResponse != null && IsValidDecimal(strCorrectResponse) == false){
		WriteToDebug("ERROR - Invalid Correct Response, not a valid decimal");
...
function SetObjectiveScore(strObjectiveID, intScore, intMaxScore, intMinScore){
	...
	if (! IsValidDecimal(intMinScore)){
		WriteToDebug("ERROR - Invalid Min Score, not a valid decimal");
...
function GetBucketInfo(strBucketId){
	...
	//bucket size needs to be halved to translate from octets to characters
